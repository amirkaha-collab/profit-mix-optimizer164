...
def _init_state():
    ...
    st.session_state.setdefault("journey_product_optimization_done",   False)
    st.session_state.setdefault("journey_portfolio_optimization_done", False)
...
.pb-outer{
  ...
  padding:12px 20px 10px;margin-bottom:20px;direction:rtl;
  box-shadow:0 1px 4px rgba(11,25,41,.04);
}
.pb-label{
...
.pb-item:not(:last-child)::after{
  content:'';position:absolute;top:14px;
  right:calc(-50% + 14px);
  width:calc(100% - 28px);height:2px;
  background:var(--line,#e2e8f0);z-index:0;
}
.pb-item.done::after,.pb-item.active::after{
  --line:#2563eb;
}
.pb-node{
  ...
  background:var(--nbg,#f1f5f9);
  border:2px solid var(--nbd,#e2e8f0);
  color:var(--nc,#94a3b8);
  transition:all .2s;
}
.pb-item.done  .pb-node{--nbg:#eff6ff;--nbd:#2563eb;--nc:#2563eb}
.pb-item.active .pb-node{
  --nbg:#0f2d5e;--nbd:#0f2d5e;--nc:#fff;
  box-shadow:0 0 0 4px rgba(15,45,94,.12);
}
.pb-item-lbl{
  font-size:10.5px;font-weight:600;margin-top:6px;
  color:var(--lc,#94a3b8);text-align:center;
  white-space:nowrap;
}
.pb-item.done  .pb-item-lbl{--lc:#1d4ed8;font-weight:700}
.pb-item.active .pb-item-lbl{--lc:#0b1929;font-weight:800}
.pb-mode-badge{
  display:inline-flex;align-items:center;gap:5px;
  font-size:9.5px;font-weight:700;padding:3px 10px;border-radius:999px;
  margin-top:8px;
}
.pb-mode-full{background:#d1fae5;color:#065f46;border:1px solid #6ee7b7}
.pb-mode-partial{background:#fef3c7;color:#92400e;border:1px solid #fde68a}
...
def _journey_status() -> dict:
    ...
    done_opt  = bool(ss.get("journey_optimization_done") or
                    ss.get("journey_product_optimization_done") or
                    ss.get("journey_portfolio_optimization_done"))
...
def _render_progress_bar(active_step: int) -> None:
    """Rich persistent progress bar for workflow screens."""
    steps = _journey_status()
    _labels  = {1: "ניתוח תיק", 2: "אופטימיזציה", 3: "הסברי AI", 4: 'דו"ח סופי'}
    _icons   = {1: "1", 2: "2", 3: "3", 4: "4"}

    items = []
    for n in [1, 2, 3, 4]:
        _done = steps[n]["done"]
        _cls  = "done" if _done else ("active" if n == active_step else "")
        _node = "&#x2713;" if _done else str(n)
        items.append(
            f'<div class="pb-item {_cls}">'
            f'<div class="pb-node">{_node}</div>'
            f'<div class="pb-item-lbl">{_labels[n]}</div>'
            f'</div>'
        )

    _portfolio_done = st.session_state.get("journey_portfolio_optimization_done", False)
    _mode_cls   = "pb-mode-full" if _portfolio_done else "pb-mode-partial"
    _mode_lbl   = 'דו"ח מלא' if _portfolio_done else 'ברמת מוצר'
    _mode_badge = f'<div class="pb-mode-badge {_mode_cls}">&#x25CF;&nbsp;{_mode_lbl}</div>'

    st.markdown(
        '<div class="pb-outer">' +
        '<div class="pb-label">מסלול העבודה</div>' +
        '<div class="pb-wrap">' + "".join(items) + '</div>' +
        _mode_badge +
        '</div>',
        unsafe_allow_html=True,
    )
...
if res and recs:
    ...
    # ── STEP 2 COMPLETE: detect product vs portfolio scope ───────────────
    _has_portfolio_baseline = bool(st.session_state.get("portfolio_baseline"))
    if _has_portfolio_baseline:
        st.session_state["journey_portfolio_optimization_done"] = True
    else:
        st.session_state["journey_product_optimization_done"] = True
    st.session_state["journey_optimization_done"] = True

    # ── MODE BANNER ──────────────────────────────────────────────────────
    if not _has_portfolio_baseline:
        _mc, _ = st.columns([3, 1])
        with _mc:
            st.info(
                f"**אתה עובד ברמת מוצר** — {_product_type_now}.  \\n"
                "הדוח שייוצר יכסה עולם מוצר בודד.  \\n"
                "כדי להפיק **דוח לקוח מלא** — יש להשלים ניתוח תיק קיים.",
                icon="\u2139\ufe0f"
            )
    st.markdown("---")
...
<div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:14px;
  ...
  <div style="font-size:12.5px;color:#334155;line-height:1.6">
    ...
    _portfolio_opt_done = st.session_state.get("journey_portfolio_optimization_done", False)
    _rscope = 'דו"ח מלא לכלל התיק' if _portfolio_opt_done else f'דו"ח חלקי — {_product_type_now}'
    _rbg    = "#d1fae5" if _portfolio_opt_done else "#fef3c7"
    _rc     = "#065f46" if _portfolio_opt_done else "#92400e"
    st.markdown(f"""
<div style="background:linear-gradient(135deg,#1e293b,#0f172a);
  ...
  <div style="display:flex;align-items:center;gap:10px">
    ...
    <div>
      <div style="font-size:15px;font-weight:900;color:#f1f5f9">שלב 3 — דו"ח לקוח</div>
      <div style="font-size:11.5px;color:#94a3b8">הסברי AI · עריכה ואישור · ייצוא Notebook</div>
    </div>
  </div>
  <span style="background:{{_rbg}};color:{{_rc}};font-size:10px;font-weight:700;
    padding:3px 10px;border-radius:999px">{{_rscope}}</span>
</div>""", unsafe_allow_html=True)
    if not _portfolio_opt_done:
        _pw, _ = st.columns([3, 1])
        with _pw:
            st.warning('דוח **חלקי**: יכסה את עולם המוצר הנוכחי בלבד. לדוח מלא — קדם לניתוח תיק לקוח.')
    ...
    # ── CTA: escalate from product to full portfolio optimization ──────────
    if not st.session_state.get("journey_portfolio_optimization_done"):
        st.markdown("<div style='height:10px'></div>", unsafe_allow_html=True)
        _fwd_col, _ = st.columns([2.5, 3.5])
        with _fwd_col:
            st.markdown("""
<div style="background:#eff6ff;border:1.5px solid #93c5fd;border-radius:12px;
  ...
  <div style="font-size:11px;font-weight:700;color:#1d4ed8;letter-spacing:1.5px;
       text-transform:uppercase;margin-bottom:4px">מסלול מלא</div>
  <div style="font-size:14px;font-weight:800;color:#0b1929;margin-bottom:4px">
    המשך לבניית תמהיל כולל לתיק
  </div>
  <div style="font-size:12px;color:#475569;margin-bottom:2px">
    עבור לניתוח תיק לקוח כדי לקבל דוח מלא על כלל המוצרים.
  </div>
</div>""", unsafe_allow_html=True)
            st.markdown("<div style='height:6px'></div>", unsafe_allow_html=True)
            if st.button("📊 המשך לניתוח תיק כולל", key="opt_to_portfolio",
                         use_container_width=True, type="primary"):
                _nav_to("app", "תיק לקוח")
                st.rerun()
